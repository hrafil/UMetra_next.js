...
const Catalog2 = (lines) => {
  ...
  const handleAuthor = (e) => {
    ...
    for (let i = 0; i < artworksIn.length; i += 1) {
      let index = artworksIn.indexOf(artworksIn[i]);
      console.log(index);
      artworksIn[i].author !== selectedAuthor ? artworksIn.slice(index, 1) : '';
    }
  };

  // artworksIn.splice(1, 1);
  ...
  return (
    <div className={styles.container}>
      ...
      <div>
        {artworksIn.map((artwork) => (
          <div key={artwork.id} className={styles.containerA}>
